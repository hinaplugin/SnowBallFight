...
public class LeaveCommand {
    ...
    public boolean Leave(@NotNull CommandSender sender) {
        try {
            if (!sender.hasPermission("snowballfight.commands.leave")) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (!this.warehouse.getJoinPlayers().contains(player)) {
                        player.sendMessage(ChatColor.RED + "まだ待機室に入室していないため実行できません．");
                        return true;
                    }
                    ...
                    this.warehouse.getJoinPlayers().remove(player);
                    player.sendMessage(ChatColor.AQUA + "待機室から退室しました．");
                } else {
